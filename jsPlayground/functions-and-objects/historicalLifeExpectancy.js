...
// Takes an array of elements and separates them into different groups based on the groupOf function
function groupBy(array, groupOf) {
  // Initialize the return object
  var groups = {};
  // Assign each element to a group
  array.forEach(function(element) {
    // The name of the group is given by groupOf
    var groupName = groupOf(element);
    // If there is already an element in this group, add the element to the group
    if (groupName in groups) {
      groups[groupName].push(element);
      // If the group doesn't exist yet, create the group and add this element
    } else {
...
var byCentury = groupBy(ancestry, function(person) {
  // rounds upwards to nearest 100th ex: 1607 → 17
  return Math.ceil(person.died / 100);
});

// Log the average age for each group
for (var century in byCentury) {
  // Transform each group array into an array of ages
  var ages = byCentury[century].map(function (person) {
    return person.died - person.born;
  });
  console.log('The average historical life expectancy in the ' + century + 'th century: ' + average(ages));
}
// 16: 43.5
// 17: 51.2
// 18: 52.8
